...
#Definición de la función tal que: y(x,0)=f(L,x).
def f(x):
    return x**4
...
n,c0 = int(input("Dar la cantidad de intervalos deseados ")),2
...
Y_real = zero_function(T_data,X_data)

#Obtención de los valores de Y(x,t) de la función real.
for i in range(0,len(T)):
    for j in range(0,len(X)):
        Y_real[i][j] = Yreal(X[j],T[i])
...
for i in range(1,n+1):
    ...
    Y[0][i] = h(T[i])
    if xf>2*T[i]:
        Y[n][i] = 0.5*((xf+2*T[i])**4+(xf-2*T[i])**4)
    else:   
        Y[n][i] = 0.5*((xf+2*T[i])**4-(xf-2*T[i])**4)
...
TextFile = open("Infinity-String-FiniteDiferentials.txt","w+")
...
ax.set_title("Solution of the Wave Equation of a Infinity String \n by Finite Differentials: \n D_tt{y(x,t)}=4D_xx{y(x,t)}")
...
ax.set_title("Real solution of the Wave Equation of a Infinity String \n by Finite Differentials: \n D_tt{y(x,t)}=4D_xx{y(x,t)}")
ax.set_xlabel("t"),ax.set_ylabel("x"),ax.set_zlabel("psi(x,t)")
ax.plot_surface(T_data,X_data,Y_real,cmap="viridis")

#Guardado de la imagen de ambas graficas.
plt.savefig("Infinity-String-num.png")
