...
def Yreal(c,t,x):
    ...
    for i in range(0,100):
        alpha = 2/(pi*(2*i+1))
        z += (alpha**3)*np.sin((2*i+1)*pi*x)*np.cos(c*(2*i+1)*pi*t)
...
print("El problema físico indica el movimiento ondulatorio de una cuerda con dos nodos")
...
n,c0 = int(input("Dar la cantidad de intervalos deseados ")),0.1
...
TextFile = open("Simple-String-FiniteDiferentials.txt","w+")
...
ax.set_title("Solution of the Wave Equation of a Simple String \n by Finite Differentials: \n D_tt{y(x,t)}=D_xx{y(x,t)}")
...
ax.set_title("Real solution of the Wave Equation of a Simple String \n by  Finite Differentials: \n D_tt{y(x,t)}=D_xx{y(x,t)}")
ax.set_xlabel("t"),ax.set_ylabel("x"),ax.set_zlabel("psi(x,t)")
ax.plot_surface(T_data,X_data,Y_real,cmap="viridis")

#Guardado de la imagen de ambas graficas.
plt.savefig("Simple-String-num.png")
