...
%     % Jumps
     ...
     uJ    = ur      - ul;      % [u]
     uxJ   = uxr     - uxl;      % [u_x]
     buxJ  = br*uxr  - bl*uxl;   % [bu_x]
     bxuxJ = bxr*uxr - bxl*uxl;  % [b_xu_x]
     %-----------
...
%     % At point x=x(J)
     ...
     B(2,1) = h2l;
     B(2,2) = hl;
     B(2,3) = hr*bl/br+0.5*(hr^2)*((bxl-a)/br-bl*(bxr-a)/(br^2));
     B(3,1) = 0.5*(h2l^2);
     B(3,2) = 0.5*(hl^2);
     B(3,3) = 0.5*(hr^2)*bl/br;
     Bb(1)  = 0;
     Bb(2)  = bxl-a;
     Bb(3)  = bl;
%     %-----------
     gam = B\Bb;
     CJ  = (uJ+hr*buxJ/br-(0.5*hr^2)*(bxr-a)*buxJ/(br^2)+(0.5*hr^2)*fJ/br)*gam(3);
%     %--------------------------------
...
%     % At point x=x(J+1)
     ...
     B(1,2) = 1;
     B(1,3) = 1;
     B(2,1) = hl*br/bl+0.5*(hl^2)*((bxr-a)/bl-br*(bxl-a)/(bl^2));
     B(2,2) = hr;
     B(2,3) = h2r;
     B(3,1) = 0.5*(hl^2)*br/bl;
     B(3,2) = 0.5*(hr^2);
     B(3,3) = 0.5*(h2r^2);
     Bb(1)  = 0;
     Bb(2)  = bxr-a;
...
%     %-----------
     gam  = B\Bb;
     CJp1 = (-uJ-hl*buxJ/bl+0.5*(hl^2)*(bxl-a)*buxJ/(bl^2)+0.5*(hl^2)*fJ/bl)*gam(1);
